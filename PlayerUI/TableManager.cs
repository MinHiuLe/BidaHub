...
namespace QuanLyBanBida
{
    public partial class f_TableManager : Form
    {
        ...
        private Dictionary<int, double> tableStartTimes = new Dictionary<int, double>();
        ...
        private void Btn_Click(object sender, EventArgs e)
        {
            ...
            // Load the start time for the selected table
            if (tableStartTimes.ContainsKey(tableID))
            {
                txt_StartTime.Text = tableStartTimes[tableID].ToString();
            }
            else
            {
                txt_StartTime.Text = "";
        ...
        {
            Table currentTable = lsvBill.Tag as Table;

            int id1 = currentTable.ID;
            ...
            // Lưu thời gian bắt đầu cho bàn hiện tại trước khi chuyển
            if (tableStartTimes.ContainsKey(id1))
            {
                double currentTableStartTime = tableStartTimes[id1];
                txt_StartTime.Text = currentTableStartTime.ToString();
            }

            if (MessageBox.Show(string.Format("Bạn có thật sự muốn chuyển bàn {0} qua bàn {1}", currentTable.Name, (cb_SwitchTable.SelectedItem as Table).Name), "Thông báo", MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
            {
        ...
        private void btn_Luu_Click(object sender, EventArgs e)
        {
            ...
            // Save the start time for the selected table
            if (double.TryParse(txt_StartTime.Text, out double enteredTime))
            {
                if (tableStartTimes.ContainsKey(table.ID))
                {
                    tableStartTimes[table.ID] = enteredTime;
                }
                else
                {
                    tableStartTimes.Add(table.ID, enteredTime);
                }

                MessageBox.Show("Đã lưu thời gian bắt đầu cho bàn " + table.Name);
            }
            else
            {
                MessageBox.Show("Vui lòng nhập một giá trị số hợp lệ.");
            }
        }
    }
}
